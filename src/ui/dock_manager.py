...
from PySide6.QtCore import (
    QEasingCurve, QEvent, QObject, QPoint, QPropertyAnimation, QRect, Qt, QTimer, Signal, Slot,
)
from PySide6.QtGui import QCursor, QMouseEvent
from PySide6.QtWidgets import (
    QApplication, QDockWidget, QFrame, QHBoxLayout, QLabel, QMainWindow, QStyle, QToolButton, QWidget,
)
...
class DockManager(QObject):
    ...
    def __init__(
        ...
        *,
        ...
        self._preview_animation = QPropertyAnimation(self._preview, b"geometry", self)
        self._preview_animation.setDuration(self.PREVIEW_MS)
        self._preview_animation.setEasingCurve(QEasingCurve.Type.OutCubic)
    ...
    def restore_widths(self) -> None:
        ...
        try:
            docked = [d for d in self._docks() if d.isVisible() and (not d.isFloating())]
            if len(docked) == 2 and self._area(docked[0]) == self._area(docked[1]):
                self.main_window.resizeDocks(docked, [self._states[d].width for d in docked], Qt.Orientation.Horizontal)
            else:
                for dock in docked:
                    self.main_window.resizeDocks([dock], [self._states[dock].width], Qt.Orientation.Horizontal)
        finally:
            self._restoring = False
    ...
    def _configure_main_window(self) -> None:
        ...
        self.main_window.setDockOptions(QMainWindow.DockOption.AnimatedDocks | QMainWindow.DockOption.AllowNestedDocks)
    ...
    def _configure_dock(self, dock: QDockWidget) -> None:
        ...
        # Native Dock-Bewegung absichtlich deaktivieren: Nur unsere linken/rechten
        # Drop-Ziele sollen aktiv sein, keine Qt-Vorschau oben oder unten.
        dock.setFeatures(features & ~QDockWidget.DockWidgetFeature.DockWidgetMovable)
        dock.setAllowedAreas(self.NO_NATIVE_DOCK)
        dock.installEventFilter(self)
        title_bar = _DockTitleBar(dock.windowTitle(), closable, dock)
        dock.setTitleBarWidget(title_bar)
        self._title_bars[dock] = title_bar
        title_bar.drag_started.connect(lambda pos, d=dock: self._start_drag(d, pos))
        title_bar.drag_moved.connect(lambda pos, d=dock: self._move_drag(d, pos))
        title_bar.drag_finished.connect(lambda pos, d=dock: self._finish_drag(d))
        title_bar.float_requested.connect(lambda d=dock: self._toggle_floating(d))
    ...
    def _connect_dock(self, dock: QDockWidget) -> None:
        dock.visibilityChanged.connect(lambda visible, d=dock: self._emit_visibility(d, visible))
        dock.topLevelChanged.connect(lambda floating, d=dock: self._top_level_changed(d, floating))
        dock.dockLocationChanged.connect(lambda area, d=dock: self._remember_area(d, area))

    # --- Docking / Floating ---------------------------------------------

    def _dock_menu(self, dock: QDockWidget, area: Qt.DockWidgetArea) -> None:
        ...
        other_docked = other.isVisible() and (not other.isFloating()) and (self._area(other) in self.VALID_AREAS)
        ...
        try:
            ...
            if other_docked and other_area == area:
                ...
                self.main_window.splitDockWidget(first, second, Qt.Orientation.Horizontal)
            else:
    ...
    def _add_docked(self, dock: QDockWidget, area: Qt.DockWidgetArea) -> None:
        ...
        # Qt kann einen alten Floating-/Split-Zustand behalten. In diesem Fall
        # wird die gewünschte Position bewusst ein zweites Mal erzwungen.
    ...
    def _pair_order(self, area: Qt.DockWidgetArea) -> tuple[QDockWidget, QDockWidget]:
        return (self.navigation, self.detail) if area == self.LEFT else (self.detail, self.navigation)

    def _float(self, dock: QDockWidget, top_left: QPoint | None=None) -> None:
        self._remember_width(dock)
        height = max(1, dock.height())
        dock.setAllowedAreas(self.NO_NATIVE_DOCK)

        # Vor setFloating(True) die Custom-Titlebar entfernen, damit Windows
        # einen echten nativen Fensterrahmen erzeugt.
        self._use_native_title_bar(dock)
        dock.setFloating(True)
        dock.resize(self._states[dock].width, height)

        # Floating-Fenster dürfen nur horizontal skaliert werden.
    ...
    def _poll_release(self) -> None:
        ...
        if self._native_floating_drag:
            ...
            self._finish_drag(dock)

    def _finish_drag(self, dock: QDockWidget) -> None:
        if self._dragging is not dock:
            return
        target = self._target
        ...
        if target is not None:
            if target.other is None:
                ...
                self._dock_beside(dock, target.other, target.side)
            return
        if was_floating:
            dock.move(final_pos)
            dock.show()
    ...
    def _dock_beside(self, dock: QDockWidget, other: QDockWidget, side: str) -> None:
        area = self._area(other)
        if area not in self.VALID_AREAS:
            return
        visible = {dock: dock.isVisible(), other: other.isVisible()}
        first, second = (dock, other) if side == "left" else (other, dock)
        self._layout_busy = True
        try:
            self.main_window.removeDockWidget(dock)
            self.main_window.removeDockWidget(other)
    ...
    def _dock_at_central_edge(self, dock: QDockWidget, area: Qt.DockWidgetArea) -> None:
        other = self._other(dock)
        if other.isVisible() and (not other.isFloating()) and (self._area(other) == area):
            self._dock_beside(dock, other, "right" if area == self.LEFT else "left")
        else:
    ...
    def _target_other(self, dock: QDockWidget, cursor: QPoint) -> _Target | None:
        other = self._other(dock)
        ...
        if not rect.top() <= cursor.y() <= rect.bottom():
            return None
        left = QRect(rect.left() - self.EDGE_ZONE, rect.top(), self.EDGE_ZONE * 2, rect.height())
        right = QRect(rect.right() - self.EDGE_ZONE + 1, rect.top(), self.EDGE_ZONE * 2, rect.height())
    ...
    def _preview_rect(self, dock: QDockWidget, target: _Target) -> QRect | None:
        width = max(self.PREVIEW_MIN_WIDTH, self._states[dock].width)
        if target.other is not None:
            rect = self._rect_in_main(target.other)
            width = min(width, max(self.PREVIEW_MIN_WIDTH, self.main_window.width() // 2))
            if target.side == "left":
        ...
        width = min(width, max(self.PREVIEW_MIN_WIDTH, rect.width() - 40))
        x = rect.left() if target.area == self.LEFT else rect.right() - width + 1
        return QRect(x, rect.top(), width, rect.height())
    ...
    def _animate_preview(self, target: QRect, side: str) -> None:
        ...
        if self._preview.isVisible():
            ...
            x = target.left() if side == "left" else target.right() - collapsed + 1
            start = QRect(x, target.top(), collapsed, target.height())
    ...
    def eventFilter(self, watched: QObject, event: QEvent) -> bool:
        ...
        # Floating verwendet die native Windows-Titelleiste. Deren Bewegung läuft
        # deshalb nicht über _DockTitleBar und wird hier separat beobachtet.
        if dock.isFloating():
            if event_type == QEvent.Type.NonClientAreaMouseButtonPress:
                ...
                if QApplication.mouseButtons() & Qt.MouseButton.LeftButton:
                    if self._dragging is None:
                        self._begin_native_floating_drag(dock)
                    if self._dragging is dock and self._native_floating_drag:
                        self._update_target(dock, QCursor.pos())
            elif event_type == QEvent.Type.NonClientAreaMouseButtonRelease:
                if self._dragging is dock and self._native_floating_drag:
                    self._finish_native_floating_drag(dock)
        if event_type == QEvent.Type.Resize and (not self._layout_busy) and (self._dragging is None):
            self._width_timer.start()
        return super().eventFilter(watched, event)

    def _begin_native_floating_drag(self, dock: QDockWidget) -> None:
        if self._dragging is not None:
            return
        self._remember_width(dock)
        self._dragging = dock
    ...
    def _finish_native_floating_drag(self, dock: QDockWidget) -> None:
        if self._dragging is not dock or not self._native_floating_drag:
            return
        target = self._target
        self._dragging = None
        self._target = None
    ...
    def _schedule_restore(self) -> None:
        # Mehrere Durchläufe sind absichtlich nötig, weil Qt Dock-Layouts
        # zeitversetzt nachberechnet.
        for delay in (0, 60, 140):
            QTimer.singleShot(delay, self.restore_widths)

    def _top_level_changed(self, dock: QDockWidget, floating: bool) -> None:
    ...
    def _remember_area(self, dock: QDockWidget, area: Qt.DockWidgetArea) -> None:
        if area in self.VALID_AREAS:
            self._states[dock].last_area = area

    def _use_native_title_bar(self, dock: QDockWidget) -> None:
        """Floating: echte native Windows-Fensterdekoration verwenden."""
        if dock.titleBarWidget() is not None:
            dock.setTitleBarWidget(None)

    def _use_custom_title_bar(self, dock: QDockWidget) -> None:
        """Angedockt: eigene Titelleiste für horizontal-only Drag verwenden."""
        title_bar = self._title_bars.get(dock)
        if title_bar is not None and dock.titleBarWidget() is not title_bar:
            dock.setTitleBarWidget(title_bar)
    ...
    def _emit_visibility(self, dock: QDockWidget, visible: bool) -> None:
        (self.navigation_visibility_changed if dock is self.navigation else self.detail_visibility_changed).emit(visible)

    def _emit_floating(self, dock: QDockWidget, floating: bool) -> None:
        (self.navigation_floating_changed if dock is self.navigation else self.detail_floating_changed).emit(floating)
    ...
    def _area_or(self, dock: QDockWidget, fallback: Qt.DockWidgetArea) -> Qt.DockWidgetArea:
    ...
    def _rect_in_main(self, widget: QWidget) -> QRect:
        return QRect(self.main_window.mapFromGlobal(widget.mapToGlobal(QPoint(0, 0))), widget.size())
    ...
    def _validate_area(cls, area: Qt.DockWidgetArea) -> None:
        if area not in cls.VALID_AREAS:
            raise ValueError("Sidebars dürfen nur links oder rechts angedockt werden.")